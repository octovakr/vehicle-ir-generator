...
/**
 * Microphone mounting descriptor. Consumed by the acoustic solver
 * (microphoneMounting.ts) — not a UI-only label (rules 9 / 29).
 *
 * `free` is an ideal omnidirectional point receiver (no local rigid body).
 * Every other option attaches a finite circular rigid baffle at a documented
 * mounting surface. See microphoneMounting.ts for the physical model and
 * its approximations.
 */
...
export interface MicrophoneConfig {
  ...
  /**
   * Look direction of the capsule, unitless vector in simulation coordinates.
   * Visualized in the 3D scene. The rigid-baffle plane uses the mounting
   * surface's geometric normal, not this vector — measured microphone
   * directivity (cardioid / array / etc.) is not modeled.
   */
  orientation: Vec3;
}
...
export interface ImageSourceContribution {
  ...
  /**
   * Image-source coordinates used for this arrival, meters.
   * Cabin-wall images are in Allen–Berkley unfolded space; local baffle
   * images are the incident image reflected across the mounting plane.
   */
  imagePosition: Vec3;
}
...
export interface ImpulseResponseMetadata {
  ...
  microphoneMounting: MicrophoneMounting;
  microphoneOrientation: Vec3;
  /**
   * Local rigid-baffle parameters actually consumed by the solver.
   * `null` when the microphone is a free-field point receiver.
   */
  microphoneBaffle: MicrophoneBaffleMetadata | null;
}

/** Snapshot of the finite-baffle receiver model for IR reproducibility. */
export interface MicrophoneBaffleMetadata {
  /** Characteristic disk radius `a`, meters. */
  radiusMeters: number;
  /** Energy absorption coefficient β of the mounting face. */
  absorptionCoefficient: number;
  /** Signed capsule distance from the mounting plane, meters (positive = cabin side). */
  standoffMeters: number;
  /** First-order ka high-pass cutoff f_c = c / (2π a), Hz. */
  cutoffHz: number;
  /** Extra image-source arrivals generated by the local baffle. */
